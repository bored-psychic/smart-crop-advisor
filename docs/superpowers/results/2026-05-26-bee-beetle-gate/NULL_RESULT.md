...
The 12.3% test-fold Bee→Beetle confusion did **not** reproduce on held-out iNat clips: held-out Bee→Beetle = **1.1%** (1 / 93 decided), well below the pre-committed 5% floor for proceeding to gate work. The original test-fold signal was almost certainly a small-fold artifact (9/73 = 12.3% has a Wilson 95% CI of roughly [5.9%, 22.0%] — wide enough to overlap noise on small n). Held-out Beetle→Bee = 2.2% (2 / 93), close to the test-fold 2.7%, which validates that the test-fold *reverse* direction was real but already too small to gate.
...
1. **Small-fold noise.** 9/73 has a Wilson 95% CI of ≈[5.9%, 22.0%]. The point estimate is unreliable for any single off-diagonal cell. The reverse-direction estimate (2.7%) didn't move, but that doesn't mean any single cell is well-estimated — only that *this particular cell* happened to be artifact-high.
...
(3) is the smallest defensible next step. The Bee→Beetle outcome here implies the *selection criterion* (largest test-fold off-diagonal) is unreliable, not just that this particular cell was misjudged.
