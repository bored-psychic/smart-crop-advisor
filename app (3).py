...
CROP_KC = {
    ...
    'Tomato':  {'Initial': 0.40, 'Development': 0.80, 'Mid-season': 1.15, 'Late season': 0.70},
    'Potato':  {'Initial': 0.45, 'Development': 0.75, 'Mid-season': 1.15, 'Late season': 0.75},
    'Cotton':  {'Initial': 0.35, 'Development': 0.70, 'Mid-season': 1.20, 'Late season': 0.50},
    'Soybean': {'Initial': 0.40, 'Development': 0.80, 'Mid-season': 1.15, 'Late season': 0.50},
    'Onion':   {'Initial': 0.50, 'Development': 0.70, 'Mid-season': 1.05, 'Late season': 0.75},
}
...
def calculate_ET0(temp, humidity, wind_speed):
    """Simplified FAO Penman-Monteith ET0 (mm/day)"""
    # Saturation vapor pressure
    es = 0.6108 * np.exp(17.27 * temp / (temp + 237.3))
    ea = es * humidity / 100
    vpd = es - ea
    ET0 = 0.408 * (0.0023 * (temp + 17.8) * wind_speed ** 0.5) + 0.34 * vpd
    return max(ET0, 0.5)
...
with tab4:
    ...
    with col1:
        ...
        irr_crop = st.selectbox("Crop", list(CROP_KC.keys()))
